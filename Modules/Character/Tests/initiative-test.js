...
assert(C:NextTurn());assert(C.initiative.phase=='transition' and C.initiative.round==2 and advances==0)
assert(not C:NextTurn());assert(#queue==1)
queue[1]();assert(C.initiative.phase=='play' and advances==1 and C.initiative.currentIndex==1 and C.initiative.round==2)
...
C.initiative.isHost=true;C:NextTurn();C:NextTurn();local stale=queue[2];C.initiative._pendingRound=nil;C.initiative.phase='play';stale();assert(C.initiative.phase=='play')
